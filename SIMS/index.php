<?php include('allhead.php'); ?>

<!-- Header Carousel -->
<header id="myCarousel" class="carousel slide">
	<!-- Indicators -->
	<ol class="carousel-indicators">
		<li data-target="#myCarousel" data-slide-to="0" class="active"></li>
		<li data-target="#myCarousel" data-slide-to="1"></li>
		<li data-target="#myCarousel" data-slide-to="2"></li>
	</ol>

	<!-- Wrapper for slides -->
	<div class="carousel-inner">
		<div class="item active">
			<div class="fill" style="background-image:url('images/1900x10801.png');"></div>
			<div class="carousel-caption">
				<a href="registrationform"><h2 style="color: white;">Register Today</h2></a>
			</div>
		</div>
		<div class="item">
			<div class="fill" style="background-image:url('images/1900x10802.png');"></div>
			<div class="carousel-caption">
				<h2>Feed your Brain With Knowledge</h2>
			</div>
		</div>
		<div class="item">
			<div class="fill" style="background-image:url('images/1900x10803.png');"></div>
			<div class="carousel-caption">
				<h2 style="color: white; ">Work Conveniently</h2>
                </div>
            </div>
        </div>

        <!-- Controls -->
        <a class="left carousel-control" href="#myCarousel" data-slide="prev">
            <span class="icon-prev"></span>
        </a>
				<a class="right carousel-control" href="#myCarousel" data-slide="next">
            <span class="icon-next"></span>
        </a>
			
</header>

<!-- Page Content -->
<div class="container" style="max-width: 1200px;">

	<!-- Marketing Icons Section -->
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">
                   <b> WELCOME TO STUDENT INFORMATION MANAGEMENT SYSTEM</b>
                </h1>
		
		</div>
		<div class="col-md-4">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4><i class="fa fa-fw fa-check"></i><b> About Us</b></h4>
				</div>
				<div class="panel-body">
					<ul style="list-style-type:disc">
					
					<li><p>A Student Information Management System (SIMS) is a software application 
						used by educational institutions to manage and organize student-related information. 
						It is typically used by schools, colleges, and universities to streamline administrative processes, enhance communication, and improve student record management.
						 SIMS generally include a range of features and functionalities that help educational institutions manage student data effectively.</p></li>
					
					</ul>
						
					<!-- <a href="about" class="btn btn-default">Learn More</a> -->
				</div>
			</div>
		</div>
		<div class="col-md-4">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4><i class="fa fa-fw fa-gift"></i> <b>Objectives</b></h4>
				</div>
				<div class="panel-body">
					<p>
					<ul style="list-style-type:disc">
					    <li>Student Enrollment.</li>
						<li>Keep records of all Students,Teachers and Faculty.</li>
						<li>Reporting and Analytics.</li>
						<li>For making accurate records.</li>
						<li>Communication and Collaboration.</li>
						<li>Improved Administrative Processes.</li>
						<li>Data Security and Privacy.</li>
						<li>To lead the institute on to a path of flexible work i.e. work from anywhere.<br></li>
						
					</ul>
					 </p>
					<!-- <a href="about" class="btn btn-default">Learn More</a> -->
				</div>
			</div>
		</div>
		<div class="col-md-4">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4><i class="fa fa-fw fa-compass"></i><b> Technology Used</b></h4>
				</div>
				<div class="panel-body">
					<p>
					<ul style="list-style-type:disc">
					<li>RDBMS (Relational Database Management System)</li>
					<li>BOOTSTRAP</li>
					<li>Html and CSS</li>
					<li>JavaScript</li>
					<li>APACHE SERVER</li>
					<li>PHP</li>
					<li>MYSQL</li>
					<li>Python</li>
					<li>Machine Learning</li>
					</ul>
					</p>
					<!-- <a href="about" class="btn btn-default">Learn More</a> -->
				</div>
			</div>
		</div>
	</div>
	<!-- /.row -->

	<!-- Portfolio Section -->
	<!-- <div class="row">
		<div class="col-lg-12">
			<h2 class="page-header">TECHNOLOGY USED</h2>
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/apache_webserver_logo.png" alt="">
                </a>
		
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/14522157584_a67ab2e898_o.png" alt="">
                </a>
		
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/687474703a2f2f707265636973696f6e2d736f6674776172652e636f6d2f77702d636f6e74656e742f75706c6f6164732f323031342f30342f6a5175726572792e676966.gif" alt="">
                </a>
		
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/Javascript-shopping-cart.png" alt="">
                </a>
		
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/maxresdefault.jpg" alt="">
                </a>
		
		</div>
		<div class="col-md-4 col-sm-6">
			<a href="#">
                    <img class="img-responsive img-portfolio img-hover" src="images/3271.jpg" alt="">
                </a>
		
		</div>
	</div>
	 /.row -->

	<!-- Features Section -->
	<!-- <div class="row">
		<div class="col-lg-12">
			<h2 class="page-header">INSTITUTE OF CURRENT STUDENTS</h2>
		</div>
		<div class="col-md-6">
			<p>The INSTITUTE OF CURRENT STUDENTS by <strong>VISHAL MATHUR 146891645</strong> includes:</p>
			<ul>
				<li><strong>Bootstrap v3.3.7</strong>
				</li>
				<li>jQuery v1.11.1</li>
				<li>Font Awesome v4.2.0</li>
				<li>Working PHP with validation</li>
				<li>Unstyled page elements for easy customization</li>
				<li>50 PHP pages</li>
			</ul>
			
		</div>
		<div class="col-md-6">
			<img class="img-responsive" src="images/7fb1f193435815a86c8484f82b9589e1.jpg" alt="">
		</div>
	</div>
	 --> 


	<!-- <hr> -->


	<?php include('allfoot.php'); ?>